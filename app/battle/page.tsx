...
function BattleAlert({ alert }: { alert: any }) {
...
function BattlePageInner() {
  ...
  const questionStartedAtRef = useRef<any>(null);
  const deploymentTimerRef = useRef<any>(null);
  const pendingBoardRef = useRef<{ board: any; points: number } | null>(null);

  // Equipment pricing & icons list (mines are free — hidden danger)
  const unitSpecs: Record<
    string,
  ...
  const { playGameSound, triggerCombatEventSound, playActionHitSound } =
    useBattleAudio();
  ...
  useEffect(() => {
    ...
    const qSeconds =
      Number(currentQ?.timer_seconds) > 0
        ? Number(currentQ?.timer_seconds)
        : 60;
  ...
  useEffect(() => {
    ...
    const currentQ = (questions || []).find(
      (q) => q.id === room.active_question_id,
    );
    const totalSeconds =
      Number(currentQ?.timer_seconds) > 0
        ? Number(currentQ?.timer_seconds)
  ...
  const getActiveQuestionTimerSeconds = () => {
    const currentQ = (questions || []).find(
      (q) => q.id === room?.active_question_id,
    );
    return Number(currentQ?.timer_seconds) > 0
      ? Number(currentQ?.timer_seconds)
      : 60;
  };
  ...
  const getTeamUrl = (rId: string, tIndex: number) => {
    const token =
      tIndex === 1
  ...
  const loadDatabaseData = useCallback(async () => {
    ...
    try {
      ...
      let visibleTeams = (tData || []).map((team: any) => ({
        ...team,
        board: [],
      }));
          ...
          setCombatEvents((previous) => {
            ...
            const filtered = (previous || []).filter((e) => {
              ...
              if (
                ...
                Number(e.target_team_index) ===
                  Number(newEvent.target_team_index) &&
  ...
  const handleResolveDraw = (questionId: string) =>
    runAction(async () => {
      const { error } = await supabase.rpc("resolve_room_question", {
        p_room_id: roomId,
        p_question_id: questionId,
  ...
  const handleGrantExtraStrike = (grantTeamIndex: number, count = 1) =>
    runAction(async () => {
  ...
  const handleGrantPoints = (grantTeamIndex: number, points: number) =>
    runAction(async () => {
  ...
  const handleSetCurrentTurn = (targetTeamIndex: number) =>
    runAction(async () => {
  ...
  // called it out
  const handleStrike = (attackerTeamIndex: number, cellIndex: number) =>
    runAction(async () => {
      ...
      try {
        ...
        if (error) {
          ...
          if (
            ...
            console.warn(
              "Cell was already struck in DB, keeping struck state.",
            );
            return;
          }
  ...
  const handleCancelStrike = (attackerTeamIndex: number) =>
    runAction(async () => {
  ...
  if (!mounted || authLoading) {
    return (
      <div
        className="min-h-screen  flex items-center justify-center dir-rtl"
        ...
        <main className="max-w-[85rem] mx-auto px-4 mt-8 flex-grow grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white p-5 sm:p-6 rounded-3xl border border-slate-200/80 shadow-md flex flex-col relative overflow-hidden transition-all hover:shadow-lg">
                ...
                <div className="mt-4 space-y-3 flex-1">
                  ...
                  {/* Compact QR Code & Share row on single card line */}
          ...
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-md">
              ...
              {/* Categories listed with large image and clear text */}
              ...
              <div className="grid grid-cols-1 gap-3.5">
                {Object.keys(unitSpecs).map((key) => {
                  ...
                  return (
                    <button
                      key={key}
                      onClick={() => setSelectedUnit(key as any)}
                      disabled={activeTeam.is_ready}
