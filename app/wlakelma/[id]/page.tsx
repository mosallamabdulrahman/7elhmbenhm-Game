...
export default async function WlaKelmaPage({ params }: PageProps) {
  ...
  if (questionId) {
    try {
      ...
      if (roomAnswer) {
        ...
        // 2. Fallback: try room_questions directly
  ...
  return (
    <div
      dir="rtl"
      className="min-h-[100dvh] bg-[#FCFDFF] flex items-center justify-center p-4 sm:p-6 text-slate-900 font-sans select-none"
    >
      {/* Main Card Container with Cyan Border matching Game Screen */}
      <div className="relative w-full max-w-md bg-white border-4 border-cyan-500 rounded-[2.2rem] sm:rounded-[2.8rem] pt-8 sm:pt-10 pb-6 px-4 sm:px-6 flex flex-col items-center text-center shadow-2xl my-auto">
        {/* Game Logo positioned directly on the top border line */}
        ...
        {found ? (
          <div className="w-full flex flex-col items-center">
            {/* Title / Headline */}
            <h1 className="text-xl sm:text-2xl md:text-3xl font-bold text-slate-900 tracking-tight leading-snug mt-2 mb-4 px-2">
              {answerText}
            </h1>
