...
export const DIFFICULTY_STRIKE_LABEL: Record<string, string> = {
  easy: "طقة وحدة",
  medium: "طقتين",
  hard: "ثلاث طقات",
};
...
interface LimitedPlaybackMediaProps {
  mediaUrl: string;
        ...
        {sorted.map((team) => {
          ...
          return (
            ...
            >
              ...
              <p
                className={`text-3xl font-bold mt-1 ${isWinner ? "text-amber-300" : "text-white/80"}`}
              >
                {team.score}
              </p>
...
export function ScoreCards({ teams }: { teams: any[] }) {
  return (
...
interface QuestionSlotButtonProps {
  question?: any;
  difficulty: string;
  side: "left" | "right";
  row: number;
