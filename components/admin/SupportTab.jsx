...
export default function SupportTab({ notify, onRefreshUnread }) {
  const [messages, setMessages] = useState([]);
  const [loading, setLoading] = useState(true);
  const [busy, setBusy] = useState(false);
  const [searchQuery, setSearchQuery] = useState("");
  const [filterStatus, setFilterStatus] = useState("all"); // all, unread, read, resolved
                    ...
                    <td className="p-3">
                      {msg.room_id ? (
                        <button
                          ...
                          className="inline-flex items-center gap-1 text-[11px] font-mono bg-slate-100 hover:bg-slate-200 text-slate-700 px-2 py-0.5 rounded cursor-pointer transition"
                  ...
                  {previewMsg.status !== "resolved" && (
                    <button
                      ...
                      onClick={() => {
                        ...
                        setPreviewMsg((p) => ({ ...p, status: "resolved" }));
                      }}
