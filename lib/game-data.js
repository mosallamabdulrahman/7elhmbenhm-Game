...
const fetchActiveQuestions = async (supabase) => {
  const fetchPage = async (columns, from, to) => {
    return supabase
      .from("question_bank")
      .select(columns)
      .eq("is_active", true)
  ...
  const testFirst = await fetchPage(`${QUESTION_BANK_COLUMNS},show_question_first`, 0, 0);
  const useOption = !testFirst.error || !isMissingSchema(testFirst.error, "show_question_first");
  const cols = useOption ? `${QUESTION_BANK_COLUMNS},show_question_first` : QUESTION_BANK_COLUMNS;
